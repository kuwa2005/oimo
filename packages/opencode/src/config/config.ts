...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in oimo config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
const InfoSchema = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for oimo serve and web commands",
  }),
  ...
  memory: Schema.optional(
    Schema.Struct({
      ...
      cc_index: Schema.optional(Schema.Boolean).annotate({
        description:
          "Index Claude Code memory (~/.claude/projects/<slug>/memory) and expose under scope='cc'. Default: false. Note: when enabled, every oimo agent (build/explore/subagents) can search these memories via the builtin `memory` tool — including CC's `type: user` (your role/preferences) and `type: feedback` (your guidance) categories. CC originally writes them for future CC sessions; flipping this on widens the consumer set to oimo agents on the same machine. Leave disabled (default) if you don't want personal context recallable from a prompt-injection-vulnerable agent.",
      }),
    }),
  ),
...
function globalConfigFile() {
  const candidates = ["oimo.jsonc", "oimo.json", "config.json"].map((file) =>
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "oimo.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "oimo.jsonc"))),
      )
      ...
      const globalConfigFile = path.join(Global.Path.config, "oimo.jsonc")
      if (
        !existsSync(path.join(Global.Path.config, "config.json")) &&
        !existsSync(path.join(Global.Path.config, "oimo.json")) &&
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.MIMOCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("oimo", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        // Load Claude Code commands first so .oimo commands override on name collision.
        for (const dir of yield* ConfigPaths.claudeCommandDirectories(ctx.directory, ctx.worktree)) {
          result.command = mergeDeep(result.command ?? {}, yield* Effect.promise(() => ConfigCommand.load(dir)))
        }

        for (const dir of directories) {
          if (dir.endsWith(".oimo") || dir === Flag.MIMOCODE_CONFIG_DIR) {
            for (const file of ["oimo.json", "oimo.jsonc"]) {
          ...
          // Auto-discovered plugins under `.oimo/plugin(s)` are already local files, so ConfigPlugin.load
